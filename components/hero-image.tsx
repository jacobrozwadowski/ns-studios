...
import { ReelContainer } from "./video/reel-container";
...
export const HeroImage = () => {
  ...
  useEffect(() => {
    const reelElement = document.getElementById("reelcontainer");
    setTimeout(() => {
      reelElement?.classList.add("opacity-100");
    }, 700);
  }, []);
  ...
  return (
    <div
      id="reelcontainer"
      ref={ref}
      className="mt-[12.8rem] opacity-0 transition-opacity ease-in-out [perspective:2000px]"
    >
      <div
        className={classNames(
          "relative rounded-lg border border-transparent-white bg-white bg-opacity-[0.01] bg-hero-gradient",
          inView ? "animate-image-rotate" : "[transform:rotateX(25deg)]",
          "before:absolute before:top-0 before:left-0 before:h-full before:w-full before:bg-hero-glow before:opacity-0 before:[filter:blur(700px)]",
